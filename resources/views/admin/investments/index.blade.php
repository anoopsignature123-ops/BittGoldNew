        ...
        <div class="card gold-card users-card detailed-users-card">
            <div class="card-body">
                <form method="GET" action="{{ route('admin.investments.index') }}" id="filter-form">
                    <div class="list-toolbar">
                        <div>
                            ...
                            <small>Showing {{ $investments->firstItem() ?? 0 }} to {{ $investments->lastItem() ?? 0 }} of {{ $investments->total() }} records</small>
                        </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="member-cell">
                                            <span class="member-avatar">{{ strtoupper(substr($inv->user->name ?? 'U', 0, 2)) }}</span>
                                            <div>
                                    ...
                                    <td class="wallet-value font-weight-bold text-success">{{ number_format($inv->amount, 2) }}</td>
                                    <td>
                                        <span class="status-badge status-active">{{ ucfirst($inv->status) }}</span>
                                    </td>
                                    <td>{{ $inv->activated_at ? \Carbon\Carbon::parse($inv->activated_at)->format('d M Y, h:i A') : 'N/A' }}</td>
                                </tr>
                            @empty
                                <tr>
                ...
                <div class="datatable-footer mt-3">
                    <span>Showing {{ $investments->firstItem() ?? 0 }} to {{ $investments->lastItem() ?? 0 }} of {{ $investments->total() }} records</span>
