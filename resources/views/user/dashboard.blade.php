...
@push('styles')
    <style>
        ...
        .db-earnings-table thead th:not(:first-child) {
            text-align: right;
        }
        ...
        .db-earnings-table tbody tr:last-child {
            border-bottom: none;
        }
        ...
        .db-quick-ref-box {
            grid-column: span 2;
            /* Ise poori row mein failane ke liye */
            background: rgba(5, 14, 28, 0.85);
            border: 1px solid rgba(184, 140, 30, 0.3);
            border-radius: 12px;
            padding: 1rem 1.1rem;
    ...
    <div class="content-wrapper">
        ...
        <section class="db-hero mb-0">
            ...
            <div class="db-hero-bg-art" aria-hidden="true">
                <svg viewBox="0 0 360 210" fill="none" xmlns="http://www.w3.org/2000/svg"
                    ...
                    <path d="M15,185 70,145 115,155 170,100 220,78 280,42 330,18 330,200 15,200 Z" fill="url(#heroGradFill)"
                        opacity="0.22" />
                    <!-- Grid lines subtle -->
                    <line x1="15" y1="60" x2="340" y2="60" stroke="rgba(245,189,50,0.07)"
                        stroke-width="1" />
                    <line x1="15" y1="110" x2="340" y2="110" stroke="rgba(245,189,50,0.07)"
                        stroke-width="1" />
                    <line x1="15" y1="160" x2="340" y2="160" stroke="rgba(245,189,50,0.07)"
                        stroke-width="1" />
                    <!-- Main chart line -->
                    <polyline points="15,185 70,145 115,155 170,100 220,78 280,42 330,18" stroke="url(#lineGrad)"
                    ...
                    <ellipse cx="300" cy="178" rx="28" ry="7" fill="rgba(245,189,50,0.1)"
                        stroke="rgba(245,189,50,0.2)" stroke-width="1" />
                    <ellipse cx="300" cy="171" rx="28" ry="7" fill="rgba(245,189,50,0.08)"
                        stroke="rgba(245,189,50,0.15)" stroke-width="1" />
                    <ellipse cx="300" cy="164" rx="28" ry="7" fill="rgba(245,189,50,0.12)"
                        stroke="rgba(245,189,50,0.25)" stroke-width="1" />
                    <!-- Star sparkles -->
        ...
        @if (($kycStatus ?? 'none') !== 'approved')
            <div class="db-card db-card-accent p-3 mb-4"
                style="background: linear-gradient(135deg, #2d1e05 0%, #1a1202 100%); border: 1px solid #f5c842;">
                <div class="d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center gap-3">
                        <div style="font-size: 2rem; color: #f5c842;">
                            <i class="mdi mdi-shield-alert-outline"></i>
                        </div>
                        <div>
                            <h5 class="text-white mb-0">KYC Verification Pending</h5>
                            <p class="text-muted mb-0" style="font-size: 0.85rem;">
                                @if (($kycStatus ?? 'none') === 'pending')
                                    Your KYC is currently under review. Please wait for admin approval.
                                @else
                                    Your KYC is not completed. Please complete it to unlock withdrawals.
                                @endif
                            </p>
                        </div>
                    </div>
                    @if (($kycStatus ?? 'none') === 'none')
                        <a href="{{ route('user.kyc.index') }}" class="btn btn-warning btn-sm fw-bold">
                            Complete KYC Now
                        </a>
                    @endif
                </div>
        ...
        <table class="db-earnings-table">
            <thead>
                <tr>
                    <th>Income Type / Metric</th>
                    <th>Today Income</th>
                    <th>Total Income / Volume</th>
                </tr>
            </thead>
            <tbody>
                {{-- Team Business Row Added --}}
                <tr>
                    <td>
                        <span class="db-income-type">
                            <i class="mdi mdi-account-group" style="color:#f5c842;"></i>
                            Total Team Business
                        </span>
                    </td>
                    <td>-</td>
                    <td class="text-warning">₹ {{ number_format((float)str_replace(',', '', $powerLeg ?? '0') + (float)str_replace(',', '', $weakerLeg ?? '0'), 2) }}</td>
                </tr>
                <tr>
                    <td>
                        <span class="db-income-type">
                ...
                <div class="db-quick-grid">
                    ...
                    <div class="db-quick-ref-box">
                        ...
                        <div class="db-quick-ref-input-group">
                            <input type="text" id="userReferralLink" class="db-quick-ref-input"
                                value="{{ route('user.register', ['ref' => $user->referral_code]) }}" readonly>
        ...
        <div class="db-bottom-row">
            ...
            <div class="db-chart-card">
                ...
                <div class="db-chart-legend">
                    <span class="db-chart-legend-item">
                        <span class="db-chart-legend-dot"
                            style="background:#f5c842; box-shadow: 0 0 5px rgba(245,200,66,0.5);"></span>
                        Today's Income
                    </span>
                    <span class="db-chart-legend-item">
            ...
            {{-- Career Rank & Leadership Salary Details --}}
            <div class="db-rank-card">
                <div class="db-card-head">
                    <h4>Career Rank &amp; Salary</h4>
                </div>
                <div class="db-rank-body flex-column align-items-start gap-3">
                    <div class="d-flex align-items-center gap-3 w-100">
                        ...
                        <div class="db-rank-info">
                            <h3>{{ $currentRank }}</h3>
                            <p class="mb-0 text-muted" style="font-size: 0.78rem;">Current Achieved Rank</p>
                        </div>
                    </div>

                    {{-- Power Leg & Weaker Leg Volume Breakdown --}}
                    <div class="w-100 p-3 rounded"
                        style="background: rgba(5,14,28,0.7); border: 1px solid rgba(184,140,30,0.25);">
                        <div class="d-flex justify-content-between mb-2">
                            <span class="text-warning fw-bold" style="font-size: 0.75rem;">Power Leg:</span>
                            <strong class="text-warning font-monospace">₹ {{ $powerLeg ?? '0.00' }}</strong>
                        </div>
                        <div class="d-flex justify-content-between">
                            <span class="text-warning fw-bold" style="font-size: 0.75rem;">Weaker Leg (40%):</span>
                            <strong class="text-info font-monospace">₹ {{ $weakerLeg ?? '0.00' }}</strong>
                        </div>
                    </div>

                    {{-- Latest Leadership Salary Info --}}
                    <div class="w-100 p-3 rounded"
                        style="background: rgba(245,189,50,0.05); border: 1px solid rgba(245,189,50,0.2);">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <span class="d-block text-gld fw-bold"
                                    style="font-size: 0.72rem; text-transform: uppercase; letter-spacing: 0.05em;">Leadership
                                    Salary</span>
                                <span class="text-white fw-bold fs-6">₹
                                    {{ number_format($leadershipDetails['amount'] ?? 0, 2) }}</span>
                            </div>
                            <div class="text-end">
                                <span class="d-block text-white mb-2 fw-bold" style="font-size: 0.68rem;">Credited Month</span>
                                <span class="badge bg-dark border border-secondary text-warning"
                                    style="font-size: 0.75rem;">{{ $leadershipDetails['month'] ?? 'No Salary' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
